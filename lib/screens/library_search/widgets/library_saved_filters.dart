  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    List<ItemActionButton> filterActions(LibraryFiltersModel filter) {
      return [
        ...moreActions,
        ItemActionButton(
          label: Text(filter.showOnHome ? context.localized.removeFromHomeShelf : context.localized.addToHomeShelf),
          icon: Icon(filter.showOnHome ? IconsaxPlusBold.home_1 : IconsaxPlusLinear.home_1),
          action: () => filterProvider.saveFilter(filter.copyWith(showOnHome: !filter.showOnHome)),
        ),
        ItemActionButton(
