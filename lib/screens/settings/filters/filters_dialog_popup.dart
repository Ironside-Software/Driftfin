...
Future<void> showFiltersDialogue(BuildContext context) {
  return showDialogAdaptive(context: context, builder: (context) => const FiltersDialog());
}
...
class FiltersDialog extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final provider = ref.watch(libraryFiltersProvider(const []).notifier);
    final userFilters = ref.watch(libraryFiltersProvider(const []));
    ...
    return ActionContent(
      child: Column(
        ...
        children: [
          Text(context.localized.libraryFiltersTitle, style: Theme.of(context).textTheme.titleLarge),
          ...
          userFilters.isEmpty
              ...
              : Flexible(
                  child: ListView(
                    shrinkWrap: true,
                    children: [
                      ...FilterSortKey.values.map((key) {
                        final filtersForKey = ref.watch(libraryFiltersByKeyProvider(key));
                        return ExpansionTile(
                          title: Row(spacing: 6, children: [Icon(key.icon), Text(key.label(context))]),
                      ...
                      Text(
                        context.localized.filter(userFilters.length),
