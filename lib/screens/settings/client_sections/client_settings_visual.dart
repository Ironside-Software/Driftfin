...
List<Widget> buildClientSettingsVisual(
  ...
  return settingsListGroup(context, SettingsLabelDivider(label: context.localized.settingsVisual), [
      ...
      trailing: IntInputField(
        ...
        onSubmitted: (value) {
          if (value != null) {
            ref
                .read(clientSettingsProvider.notifier)
                .update((current) => current.copyWith(nextUpDateCutoff: Duration(days: value)));
          }
        },
      ),
    ),
    SettingsListTile(
      ...
      trailing: IntInputField(
        ...
        placeHolder: "500",
        onSubmitted: (value) =>
            ref.read(clientSettingsProvider.notifier).update((current) => current.copyWith(libraryPageSize: value)),
      ),
    ),
    SettingsListTile(
      label: Text(
      ...
      onTap: () => ref
          .read(clientSettingsProvider.notifier)
          .update((current) => current.copyWith(pinchPosterZoom: !current.pinchPosterZoom)),
      trailing: Switch(
        value: clientSettings.pinchPosterZoom,
        onChanged: (value) =>
            ref.read(clientSettingsProvider.notifier).update((current) => current.copyWith(pinchPosterZoom: value)),
      ),
    ),
    Column(
      children: [
        SettingsListTile(
          ...
          trailing: Text(clientSettings.posterSize.toString(), style: Theme.of(context).textTheme.bodyLarge),
        ),
        Padding(
