...
import 'package:driftfin/models/playback/playback_options_dialogue.dart';
...
import 'package:driftfin/profiles/default_profile.dart';
...
import 'package:driftfin/util/streams_selection.dart';
...
class PlaybackModelHelper {
  ...
  Future<PlaybackModel?> loadNewVideo(ItemBaseModel newItem) async {
    ...
    final newModel =
        (await createPlaybackModel(null, newItem, oldModel: currentModel)) ??
  ...
  Future<PlaybackModel?> createPlaybackModel(
    ...
    try {
      ...
      if (forcedPlaybackType == PlaybackType.offline) {
        return await _createOfflinePlaybackModel(
          item,
          null,
          await ref.read(syncProvider.notifier).getSyncedItem(item.id),
          oldModel: oldModel,
  ...
  Future<void> shouldReload(PlaybackModel playbackModel) async {
    ...
    final mediaStreamsWithUrls = MediaStreamsModel.fromMediaStreamsList(
      playbackInfo.mediaSources,
      ref,
    ).copyWith(defaultAudioStreamIndex: audioIndex, defaultSubStreamIndex: subIndex);
