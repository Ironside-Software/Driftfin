...
class $DatabaseItemsTable extends DatabaseItems with TableInfo<$DatabaseItemsTable, DatabaseItem> {
  ...
  VerificationContext validateIntegrity(Insertable<DatabaseItem> instance, {bool isInserting = false}) {
    ...
    if (data.containsKey('video_file_name')) {
      context.handle(
        _videoFileNameMeta,
        videoFileName.isAcceptableOrUnknown(data['video_file_name']!, _videoFileNameMeta),
      );
    }
    ...
    if (data.containsKey('media_segments')) {
      context.handle(
        _mediaSegmentsMeta,
        mediaSegments.isAcceptableOrUnknown(data['media_segments']!, _mediaSegmentsMeta),
      );
    }
  ...
  DatabaseItem map(Map<String, dynamic> data, {String? tablePrefix}) {
    ...
    return DatabaseItem(
      ...
      trickPlayModel: attachedDatabase.typeMapping.read(
        DriftSqlType.string,
        data['${effectivePrefix}trick_play_model'],
      ),
...
class $$DatabaseItemsTableTableManager
    extends
        RootTableManager<
          _$AppDatabase,
          $DatabaseItemsTable,
    ...
    : super(
        TableManagerState(
          ...
          withReferenceMapper: (p0) => p0
              .map(
                (e) => (
                  e.readTable<$DatabaseItemsTable, DatabaseItem>(table),
                  BaseReferences<_$AppDatabase, $DatabaseItemsTable, DatabaseItem>(db, table, e),
                ),
              )
              .toList(),
...
typedef $$DatabaseItemsTableProcessedTableManager =
